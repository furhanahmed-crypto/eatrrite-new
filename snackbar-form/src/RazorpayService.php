...
final class SnackbarRazorpayService
{
    ...
    public function unitRupees(): int
    {
        return (int) $this->config['amount_rupees'];
    }

    public function createOrder(int $quantity, array $notes): array
    {
        $rupees = $this->unitRupees() * $quantity;
        if ($rupees < 1) {
        ...
        $response = $this->request('POST', 'https://api.razorpay.com/v1/orders', [
            ...
            'receipt' => 'sb_' . bin2hex(random_bytes(6)),
            'payment_capture' => 1,
            'notes' => $notes,
        ]);
